...
function visualization_line_chart_shortcode($atts, $content = null)
{
	...
	else if (strcasecmp($display, "Humidsoil") == 0 OR strcasecmp($display, "HumS") == 0)
		$display = "hourMeasured, humidsoil, dateMeasured";
	
	else if (strcasecmp($display, "Pressure") == 0 OR strcasecmp($display, "Pre") == 0)
	...
	$resultSet = $wpdb->get_results("SELECT " . $display . " FROM relevations WHERE " . $whereConditions, ARRAY_A);
	//$resultSet = $wpdb->get_results("SELECT humidity FROM relevations WHERE " . $whereConditions, ARRAY_A);
	...
	if (($wpdb->num_rows) == 0) //nothing in table 
		...
		//displaying only HUMIDITY SOIL
		else if (strpos($display, "humidsoil") != 0) {
			$content = "['Time','Humidity Soil [%]'],";
		}
	...
	foreach ($resultSet as $row) {
		...
		else if (strpos($display, "humidsoil") != 0) //for displaying humidity soil only
			$content .= "['" . $AxisX . "'," . $row['humidsoil'] . "],";
	...
	{
		$resultSet =$wpdb->get_results("SELECT temperature FROM relevations WHERE " . $whereConditions . " ORDER BY temperature ASC LIMIT 1");//get lowest temperature  for chosen date
